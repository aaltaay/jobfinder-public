...
const DEMO_HOME = ["demo city", "demo suburb", "demo port"];
const DEMO_NEARBY = ["demo metro", "demo county", "nearby demo"];
...
function scoreLocation(input: ScoreInput): MatchReason {
  ...
  if (isHybrid && DEMO_HOME.some((city) => location.includes(city))) {
    return {
      code: "location_hybrid_home",
      label: "Hybrid near Demo City home base",
      points: 28,
      evidence: input.location || "Demo City hybrid",
    };
  }

  if (isHybrid && DEMO_NEARBY.some((city) => location.includes(city))) {
    return {
      code: "location_hybrid_nearby",
      label: "Hybrid near home metro",
      points: 20,
      evidence: input.location || "nearby hybrid",
    };
  }

  if (isOnsite && DEMO_NEARBY.some((city) => location.includes(city))) {
    return {
      code: "location_nearby_onsite",
      label: "Nearby onsite — home market",
      points: 20,
  ...
  return {
    code: "location_low",
    label: "Location fit is weak vs Demo City / remote preference",
    points: 0,
